...
Public Class PURConfSO
    ...
    Private Sub gvShow_RowDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvShow.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            If Not IsNothing(hplBOMDetail) Then
                ...
                hplBOMDetail.NavigateUrl = "../Planing/PCCheckBOMPopup.aspx?height=150&width=350" & link
            ...
            If Not IsNothing(hplShowDetail) Then
                ...
                hplShowDetail.NavigateUrl = "PURConfSOPopup.aspx?height=150&width=350" & link
                hplShowDetail.Attributes.Add("title", SO)
            End If
            Dim hlSOChange As HyperLink = CType(e.Row.FindControl("hlSOChange"), HyperLink)
            If Not IsNothing(hplShowDetail) Then
                Dim link As String = ""
                Dim SO As String = String.Empty
                Dim SoType As String = String.Empty
                ...
                link = link & "&SoNo= " & SO
                link = link & "&SoSeq= " & SoSeq
                link = link & "&item= " & lblProductItemNo.Text
                hlSOChange.NavigateUrl = "DetailSOChange.aspx?height=150&width=350" & link
                hlSOChange.Attributes.Add("title", SO)
            End If
        End If
    End Sub
    ...
    Protected Sub btSave_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btSave.Click
        ...
        Dim ConfDate As String = tbPURDate.Text
        ...
        If ConfDate <> "" And lbPURCon.Text <> "" Then
            CDate(tbPURDate.Text).ToString("yyyy-MM-dd")
        ...
        Dim PCUp As String = ""
            ...
            Dim SqlOracle As String = " UPDATE " & XMDD.tblSaleItemDeliveryDetail & " " &
                " " & XMDD.PlaningGeneralConfrimDateSaleOrder & "='" & tbPURDate.Text & "' " &
    ...
    Protected Sub gvShow_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles gvShow.RowCommand
        ...
        If e.CommandName = "OnEdit" Then
            ...
            tbRemark.Text = lblPURRemark.Text
    ...
    Private Sub ShowData()
        ...
        If SelectSaleType > 0 Then
            wWC = "substr(" & XMDC.SaleOrderNo & " ,3,4 ) in(" & [String].Join("','", SelectSaleTypeWhere) & "')"
        End If
        '### ItemProperty 
        ...
        If iRowProperty > 0 Then
            ...
            SelectProperty = IMAA.ItemCategory & " in(" & [String].Join("','", SelectProperty) & "')"
        End If
        ...
        If tbSaleNo.Text <> String.Empty And tbSOSeq.Text <> String.Empty Then
            ...
            wSaleOrderNo = SaleOrderNo & " AND " & SaleSeq
        ElseIf tbSaleNo.Text <> String.Empty And tbSOSeq.Text = String.Empty Then
            wwSaleOrderNoSeq = XMDC.SaleOrderNo & "='JP" & [String].Join("','", tbSaleNo.Text) & "'"
        End If
        If tbCust.Text <> String.Empty Then
            wCust = XMDA.CustomerId & " ='" & [String].Join("','", tbCust.Text) & "'"
        End If
        If tbItem.Text <> String.Empty Then
            wItem = XMDC.Item & " Like '" & [String].Join("','", tbItem.Text) & "%'"
        End If
        If tbSpec.Text <> String.Empty Then
            wSpec = IMAAL.Specifaction & " Like '" & [String].Join("','", tbSpec.Text) & "%'"
        End If
        If tbFromDelivDate.Text <> String.Empty AndAlso tbToDelivDate.Text <> String.Empty Then
            wDocumentDateSaleDelivery = XMDK.DocumentDate & " BETWEEN TO_DATE('" & [String].Join("','", tbFromDelivDate.Text) & "','yyyy/mm/dd') AND TO_DATE('" & [String].Join("','", tbToDelivDate.Text) & "','yyyy/mm/dd')"
        End If
        If txtsFromDate.Text <> String.Empty AndAlso txtsToDate.Text <> String.Empty Then
            wDocumentDateSaleOrder = XMDA.DocumentDate & " BETWEEN TO_DATE('" & [String].Join("','", txtsFromDate.Text) & "','yyyy/mm/dd') AND TO_DATE('" & [String].Join("','", txtsToDate.Text) & "','yyyy//mm/dd')"
        End If
